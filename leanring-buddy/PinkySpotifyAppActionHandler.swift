//
//  PinkySpotifyAppActionHandler.swift
//  leanring-buddy
//
...
struct PinkySpotifyAppActionHandler: PinkyAppActionHandling {
    ...
    private static func spotifySearchAndPlay(query: String) async -> String {
        ...
        guard PinkyOpenAppActionHandler.runOpenCommand(arguments: [spotifyURL.absoluteString]) else {
        ...
        let verification = PinkyAppleScriptRunner.run("""
        ...
        let retry = PinkyAppleScriptRunner.run("""
        tell application "Spotify"
            activate
            play
            delay 0.2
    ...
    private static func spotifyPlaybackControl(_ control: PinkySpotifyPlaybackControl) -> String {
        ...
        if PinkyOpenAppActionHandler.runOpenCommand(arguments: ["-a", "Spotify"]) {
            let retry = PinkyAppleScriptRunner.run("""
            tell application "Spotify"
                \(command)
            end tell
        ...
        return "i couldn't control Spotify. check that Spotify is installed and automation is allowed for Pinky."
    }
