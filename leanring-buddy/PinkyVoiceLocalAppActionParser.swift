//
//  PinkyVoiceLocalAppActionParser.swift
//  leanring-buddy
//
...
enum PinkyVoiceLocalAppActionParser {

    static func parse(transcript: String) -> PinkyAppAction? {
    ...
    private static func spotifyAction(from candidate: String) -> PinkyAppAction? {
        ...
        let transportPatterns: [(pattern: String, action: PinkySpotifyPlaybackControl)] = [
    ...
    private static func webOpenAction(from candidate: String) -> PinkyAppAction? {
        ...
        for entry in browserPatterns {
            if let browserRaw = captureGroup(candidate, pattern: entry.pattern, group: entry.browserGroup),
               ...
               let browser = PinkyURLActionParser.normalizedBrowser(from: browserRaw),
               let url = PinkyURLActionParser.normalizedURL(from: targetRaw) {
        ...
        for pattern in urlPatterns {
            if let targetRaw = captureGroup(candidate, pattern: pattern, group: 1),
               let parsed = PinkyURLActionParser.parseURLAndBrowser(from: targetRaw) {
    ...
    private static func appOpenAction(from candidate: String) -> PinkyAppAction? {
        let pattern =
        ...
        if PinkyURLActionParser.isLikelyTabOrURLPayload(target) {
            return nil
        }

        if let parsed = PinkyURLActionParser.parseURLAndBrowser(from: target) {
            return .openURL(url: parsed.url, browser: parsed.browser, newTab: true)
        }

        let normalizedApp = PinkyKnownApplication.normalizedName(from: target)
        if PinkyKnownApplication.isBrowser(normalizedApp),
           let url = PinkyURLActionParser.normalizedURL(from: stripBrowserPrefix(from: target)) {
    ...
    private static func bareKnownAppOpenAction(from candidate: String) -> PinkyAppAction? {
        let normalized = PinkyKnownApplication.normalizedName(from: candidate)
