//
//  ClickyVoiceSessionContinuity.swift
//  leanring-buddy
//
...
enum ClickyVoiceSessionContinuity {
    ...
    private static func isClearlyNewRequest(
        ...
        if ClickyProcedureQuery.isStepByStepIntent(transcript) {
            return !sharesStepContext(transcript, session: session)
        }
    ...
    private static func meaningfulTokens(from text: String) -> Set<String> {
        let stopWords: Set<String> = [
            ...
            "now", "please", "clicky", "hey", "ok", "okay", "step", "next",
        ]
    ...
    private static func continuityCandidate(from transcript: String) -> String {
        ClickySpokenCommandText.normalizedSpokenCommandText(
