//
//  ClickyNotchScreenSupport.swift
//  leanring-buddy
//
...
enum ClickyNotchScreenSupport {
    ...
    /// Screen-space rect covering the notch and its compact Dynamic Island flanks.
    static func notchHoverRegion(on screen: NSScreen) -> NSRect {
        let physicalNotchWidth = physicalNotchWidth(on: screen) ?? 0
        let width = max(physicalNotchWidth + 96, 196)
        let height: CGFloat = 52
        let y = statusSurfaceY(for: NSSize(width: width, height: height), on: screen) - 12
