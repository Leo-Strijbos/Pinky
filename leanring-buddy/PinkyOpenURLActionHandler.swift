//
//  PinkyOpenURLActionHandler.swift
//  leanring-buddy
//
...
struct PinkyOpenURLActionHandler: PinkyAppActionHandling {
    ...
    static func openURL(_ url: URL, browser: String?, newTab: Bool) -> String {
        ...
        if let browser, PinkyKnownApplication.isBrowser(browser) {
            if newTab, openURLInBrowserTab(urlString: urlString, browser: browser) {
                let browserName = PinkyKnownApplication.displayName(for: browser)
    ...
    private static func openURLWithBundleIdentifier(_ url: URL, browser: String) -> Bool {
        if let bundleIdentifier = PinkyKnownApplication.bundleIdentifiers[browser] {
            return PinkyOpenAppActionHandler.runOpenCommand(arguments: [
                "-b",
                bundleIdentifier,
                url.absoluteString,
            ])
        ...
        return PinkyOpenAppActionHandler.runOpenCommand(arguments: [
            "-a",
            PinkyKnownApplication.displayName(for: browser),
            url.absoluteString,
        ])
    }
