//
//  PinkyVoiceCompoundCommandParser.swift
//  leanring-buddy
//
...
enum PinkyVoiceCompoundCommandParser {
    ...
    static func parse(transcript: String) -> [CompanionSessionStep]? {
        ...
        for (index, segment) in segments.enumerated() {
            guard let action = PinkyVoiceLocalAppActionParser.parse(transcript: segment) else {
                return nil
            }
    ...
    private static func bridgePhrase(for action: PinkyAppAction) -> String {
        switch action {
        case .openApp(let appName):
            return "now opening \(PinkyKnownApplication.displayName(for: appName))."
