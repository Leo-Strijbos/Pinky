...
- **AI Chat**: Claude (Sonnet 4.6 default, Opus 4.6 optional) via Cloudflare Worker proxy. **Voice (PTT)**: screen capture + vision + web search in one call (intro-only utterances skip the screenshot). **Chat**: every message uses web search — no routing gate.
